...
class YouTubeService:
    ...
    def get_channel_videos_with_stats(
        self,
        channel_id: str,
        max_results: int = 20,
        page_token: str = None,
    ...
    def get_channel_analytics(
        self, access_token: str, channel_id: str, time_range: str = "7d"
    ):
        """
        ...
        try:
            ...
            try:
                ...
                analytics_data = []
                views_data = {
                    row[0]: int(row[1]) for row in views_response.get("rows", [])
                }
                subscribers_data = {
                    row[0]: int(row[1]) for row in subscribers_response.get("rows", [])
                }
                likes_data = {
                    row[0]: int(row[1]) for row in likes_response.get("rows", [])
                }
                comments_data = {
                    row[0]: int(row[1]) for row in comments_response.get("rows", [])
                }
                watch_time_data = {
                    row[0]: int(row[1]) for row in watch_time_response.get("rows", [])
                }
                ...
                logger.info(
                    f"Successfully loaded analytics data for channel {channel_id}"
                )
    ...
    def get_video_analytics(
        self, access_token: str, video_id: str, time_range: str = "7d"
    ):
        """
    ...
    def get_my_videos(
        self, access_token: str, max_results: int = 20, page_token: str = None
    ):
        """
        ...
        try:
            ...
            return self.get_channel_videos_with_stats(
                channel_id, max_results, page_token, access_token
            )
    ...
    def get_my_channel_analytics_detailed(
        self, access_token: str, time_range: str = "7d"
    ):
        """
    ...
    def get_my_channel_analytics_summary(self, access_token: str):
        ...
        try:
            ...
            total_subscribers = sum(
                item.get("subscribers", 0) for item in analytics_data
            )
