...
async def script_to_speech(
    ...
    try:
        ...
        crud.update_script(db, script_id, {"status": ScriptStatus.PROCESSING.value})
        db.commit()
        ...
        if all_completed:
            crud.update_script(db, script_id, {"status": ScriptStatus.COMPLETED.value})
        else:
            crud.update_script(db, script_id, {"status": ScriptStatus.FAILED.value})
        db.commit()
    ...
    try:
        ...
        for scene in script.scenes:
            for voice_audio in scene.voice_audios:
                responses.append(TextToSpeechResponse(
                    audio_url=voice_audio.audio_url,
